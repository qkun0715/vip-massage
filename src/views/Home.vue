            ...
            <van-list>
                <div v-for="(item,index) in subbranchServiceList" :key="index" @click="onItemClick(item)">
                    <service-component
                            ...
                            @onAppointment="goAppointment(item)">
                    </service-component>
                </div>
            </van-list>
        </div>
...
export default {
  ...
  watch: {
    // groupsSubbranch: {
    //     handler(newV, oldV) {
    //         // this.subbranchId = newV.subbranchId ? newV.subbranchId : this.userInfo.presetSubbranchId
    //         if (newV.subbranchId) {
    //             console.log('groupsSubbranch 请求的')
    //             this.subbranchId = newV.subbranchId
    //         } else {
    //             console.log('userInfo 请求的')
  ...
  methods: {
    ...
    /** 点击预约按钮事件**/
    goAppointment(item) {
      Toast(`ok 点击对象是${item.serviceEntity.serviceName}`);
    },
    /**点击条目事件**/
    onItemClick() {
      Toast("点击条目事件");
